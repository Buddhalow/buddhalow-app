			    	...
			    	{data.cravings && data.cravings instanceof Array ?
			    		<Table>
				    		...
				    		{data.cravings.length > 0 ? data.cravings.map(row => (
				    			<TableRow>
				    				<TableCell>
				    					{row.restaurant ? <Button color="primary" component={Link} to={`/dashboard/restaurant/${row.id}`}>{row.restaurant.name}</Button> : <span>-</span>}
				    				</TableCell>
				    				<TableCell>
				    					{row.reason && <Button component={Link} to={`/dashboard/reason/${row.id}`}>{row.reason.name}</Button>}
				    				</TableCell>
				    				<TableCell>
